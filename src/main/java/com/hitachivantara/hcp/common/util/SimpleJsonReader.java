...
public class SimpleJsonReader {
	...
	public SimpleJsonReader(String content) {
		super();
	...
	public String trackSingleNodeValue(int startIndex, String nodeName) {
		...
		int is = content.indexOf(NODE_NAME, startIndex);
		int ie;
		if (is != -1) {
			is = is + nodeName.length() + 2;
			ie = content.indexOf(NODE_NAME, is);
	...
	public List<String> trackNodeValue(int startIndex, String nodeName) {
		...
		int is = content.indexOf(NODE_NAME_B, startIndex);
		int ie = -1;
		if (is != -1) {
			is = is + nodeName.length() + 2;
			ie = content.indexOf(NODE_NAME_E, is);
		...
		while (nodeValue != null) {
			...
			is = content.indexOf(NODE_NAME_B, startIndex);
			if (is != -1) {
				is = is + nodeName.length() + 2;
				ie = content.indexOf(NODE_NAME_E, is);
